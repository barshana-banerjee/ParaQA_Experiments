...
class VerbalDataset(object):
    ...
    def _cover_entities(self, uid, question, answer):
        ...
        for word in list_ent:
            if word in answer.lower().split():
                answer = answer.replace(word, ENTITY_TOKEN)
        #if any(word in answer.lower() for word in list_ent): answer.replace(word, ENTITY_TOKEN)
    ...
    def _extract_query_answer2(self, train, test):
        return [[data['query'], [data['verbalized_answer'], data['verbalized_answer_2'],data['verbalized_answer_3'],data['verbalized_answer_4'],data['verbalized_answer_5'],data['verbalized_answer_6'],data['verbalized_answer_7'],data['verbalized_answer_8']]] for data in train], \
                [[data['query'], [data['verbalized_answer'], data['verbalized_answer_2'],data['verbalized_answer_3'],data['verbalized_answer_4'],data['verbalized_answer_5'],data['verbalized_answer_6'],data['verbalized_answer_7'],data['verbalized_answer_8']]] for data in test]
    ...
    def load_data_and_fields(self, cover_entities=False, query_as_input=False):
        ...
        train, test, val = [], [], []
        # read data
        with open(self.train_path , encoding='utf-8') as json_file:
            train = json.load(json_file)

        with open(self.test_path, encoding='utf-8') as json_file:
